...
$time_init = measure(static function() use (&$ffs, $search_in) {
    $ffs->init($search_in);
});

$sindex = '';

$time_ser = measure(static function() use (&$ffs, &$sindex) {
...
$time_unser = measure(static function() use (&$ffs, $sindex) {
...
for ($i = 0; $i < $numwords; $i++) {
    ...
    for ($j = 0; $j < $wrongLetters; $j++) {
        ...
        //Удалим букву
        if ($r < 33) {
            $p = random_int(1, mb_strlen($random_word) - 1);
            $random_word = mb_substr($random_word,0, $p) . mb_substr($random_word, $p + 1);
        } elseif ($r<66) {
            //Заменим букву
            $p = random_int(1, mb_strlen($random_word)-1);
            $letter = mb_substr($letters, random_int(0, mb_strlen($letters) - 1), 1);
